...
int main(int argc, char* argv[]) {
    ...
    if (archive_offset == 0) {
        std::cerr << "Vessel Stub Error: Archive marker not found. Is this file corrupted?\n";
        return 1;
    }
    ...
    // 0. The Inspection Flag: Dump the internal filesystem for the user dynamically
    if (argc >= 2 && std::string(argv[1]) == "--vsl-extract") {
        std::cout << "Vessel Debug: Extracting internal file system for inspection...\n";
        ...
        std::string extract_cmd = "tar -xzf " + temp_tar.string() + " -C " + dump_dir.string();
        system(extract_cmd.c_str());
        fs::remove(temp_tar);
        
        std::cout << "Successfully unpacked bundle structural contents into: " << dump_dir.string() << "\n";
        return 0;
    }
    ...
    if (needs_extraction) {
        ...
        std::string extract_cmd = "tar -xzf " + temp_tar.string() + " -C " + cache_dir.string();
    ...
    if (home_dir) {
        ...
        if (actual_vsl_path != final_vsl_location.string()) {
            std::cout << "First boot detected. Installing native OS integrations...\n";
            ...
            fs::create_directories(desktop_apps_dir);
            ...
            if (!has_icon && fs::exists(res_dir) && fs::is_directory(res_dir)) {
                for (const auto &res_file : fs::directory_iterator(res_dir)) {
                    if (res_file.path().extension() == ".png" ||
                        ...
                        installed_icon_path =
                            icons_dir /
                            (app_name + res_file.path().extension().string());
        ...
        if (registry.is_open()) {
            registry << app_fancy_name << "|" << actual_vsl_path << "\n";
            registry.close();
        }
    }

    std::string exec_path = (cache_dir / "bin" / bin_file_name).string();
    ...
    int ret = system(exec_path.c_str());
    return ret;
}
